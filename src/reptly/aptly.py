...
class Aptly:
    ...
    def run(self, *args, **kwargs):
        return subprocess.run(["aptly"] + [arg for arg in args if arg], **kwargs)

    def get_raw_list(self, type):
        return (
            self.run(type, "list", "-raw", check=True, stdout=subprocess.PIPE)
            .stdout.decode("utf-8")
            .split("\n")
        )
    ...
    def snapshot_drop(self, name: str, check: bool = True):
        self.run("snapshot", "drop", name, check=check, stdout=subprocess.DEVNULL)
    ...
    def snapshot_diff(self, a: str, b: str):
        diff = self.run(
            "snapshot", "diff", a, b, check=True, stdout=subprocess.PIPE
        ).stdout
    ...
    def snapshot_sources(self, name):
        snapshot_info = self.run(
            "snapshot", "show", name, check=True, stdout=subprocess.PIPE
        )
    ...
    def mirror_update(self, name: str, *, quiet: bool = False):
        if quiet:
            extra_args = {"stdout": subprocess.DEVNULL, "stderr": subprocess.PIPE}
        else:
            extra_args = {}
        self.run(
            "mirror",
    ...
    def switch(self, distro, target, snapshot):
        self.run("publish", "switch", distro, target, snapshot, check=True)

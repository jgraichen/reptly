...
class Testly:
    ...
    def register_publication(self, name, distribution, **components):
        self.publications[(name, distribution)] = {
            "Sources": [
                {"Component": comp, "Name": snap} for comp, snap in components.items()
            ],
        }
    ...
    def publish(
        ...
        *,
        ...
        self.run(
            "publish", target, distro, origin, content, architectures, acquire_by_hash
        )
...
class TestUI:
    ...
    def update_snapshot(self, current, proposed, *, diff, source):
        return self.act(
            "update_snapshot", (current.name, proposed.name), proposed, current
        )

    def remove_snapshot(self, current, info):
        return self.act("remove_snapshot", current.name, False, current)
    ...
    def switch(self, diff, *, target, distribution, component):
        return self.act("switch", (target, distribution, component), True, False)

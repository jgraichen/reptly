...
class SelectOption:
    def __init__(
        self, caption, value, *, chars="", default=False, fallback=False, output=None
    ):
...
class PromptToolkitUi:
    ...
    def skip_switch(self):
        ...
        print(
            f"Skipping publication for {publish.target}/{publish.distribution} {component} - no change"
        )

    def include_snapshot(self, snapshot, info):
        print(info)
        return select(
            "Do you want to include this snapshot",
    ...
    def remove_snapshot(self, current, info):
        print(info)
        return select(
            "Do you want to remove this snapshot",
    ...
    def switch(self, diff, *, target, distribution, component):
        ...
        return select(
            ...
            select.Option("no", False, chars="Nn", default=True, fallback=True),
